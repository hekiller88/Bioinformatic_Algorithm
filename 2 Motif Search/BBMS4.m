...
function [bd bw pos] = BBMS4( DNA, lmer, iftrace )
% Name: BBMS4
%   Branch and Bound Median Search (P114)
% Input: 
...
%   pos - best starting postitions array
    ...
    while i > 0
        if i < lmer
            word = num2atgc(lmer, word_s);
            [~,optDis] = totalDistance(word, i, DNA);
            if optDis > bestDis
                if iftrace == 1
                    disp(sprintf('Bypass candidate: %s', num2atgc(i, word_s)));
                end
                [word_s, i] = bypass(word_s, i, lmer,4);
            else
                if iftrace == 1
                    disp(sprintf('  Pass candidate: %s', num2atgc(i, word_s)));
                end
                [word_s, i] = nextvertex(word_s, i, lmer, 4);
            end
        else
            word = num2atgc(lmer, word_s);
            [tmpPos, tmpDis] = totalDistance(word, i, DNA);
            if tmpDis < bestDis
                bestDis = tmpDis;
                bestWord = word;
                bestPos = tmpPos;
...
function str = num2atgc(prefix, mer)
% Name: num2actg
%   number to letter(A,C,T,G) with prefix limit
% Input:
%    mer : 1 - A, 2 - C, 3 - T, 4 - G
%    prefix_i: tree level
...
function [s, dis] = totalDistance(word, prefix, DNA)
% Name: totalDistance
...
%   DNA - DNA sequences matrix
    ...
    prefixWord = '';
    for i = 1:prefix
        prefixWord = strcat(prefixWord, word(i));
    end
    
    for row = 1:t
        [~, tmpDis_row] = getBestMatchS_row(prefixWord, DNA, row);
        [tmpS_row, ~] = getBestMatchS_row(word, DNA, row);
